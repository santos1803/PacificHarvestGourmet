...
if ($id == "" || $token == "") {
    ...
    if ($token == $token_tmp) {
        ...
        if ($sql->fetchColumn() > 0) {
            $sql = $con->prepare("SELECT nombre, description, precio, descuento, categorie, enlace FROM productos WHERE id=? AND activo=1 AND categorie = 'drinks'  ");
            $sql->execute([$id]);
