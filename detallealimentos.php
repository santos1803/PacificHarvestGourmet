...
if ($id == "" || $token == "") {
    ...
    if ($token == $token_tmp) {
        $sql = $con->prepare("SELECT count(id) FROM productos WHERE id=? AND activo=1 AND categoria = 'alimentos'  ");
        $sql->execute([$id]);
        if ($sql->fetchColumn() > 0) {
            $sql = $con->prepare("SELECT nombre, descripcion, precio, descuento, categoria, enlace FROM productos WHERE id=? AND activo=1 AND categoria = 'alimentos'  ");
            $sql->execute([$id]);
        ...
        <nav class="nav container">
            ...
            <div class="nav__menu" id="nav-menu">
                ...
                <li class="dropdown__item">
                    ...
                    <ul class="dropdown__menu">
                        <li>
                            <a href="./detallealimentos.php" class="dropdown__link">
                                <img class="flag_language" src="./recursos/espana.png" alt=""> Español
                            </a>
                        </li>

                        <li>
                            <a href="./detallealimentos_english.php" class="dropdown__link">
